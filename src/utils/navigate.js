...
export const navigateToCart = () => {
	...
	if(pages.length >= 2 && pages[pages.length - 2].route === "pages/cart/index"){
		my.navigateBack();
	}
  else {
		my.navigateTo({
...
export const loadBadgeCart = (ic = "") => {
	...
	const total = orderedProducts.length ? orderedProducts.reduce((acc, curr) => acc + curr.quantity, 0) : undefined;
