...
export const filterSortProductsAPI = async({
  filters, sort,
  search = "",
}) => {
	...
	if (filters.priceOption)
		switch(filters.priceOption.value){
			case "1":
				...
				break;
			case "2":
				result = result.filter((item) => item.price >= 100000 && item.price <= 200000);
				break;
			case "3":
				result = result.filter((item) => item.price >= 200000 && item.price <= 750000);
				break;
			case "4":
    ...
    result = result.filter((item) => item.price >= filters.priceRange.start.value && item.price <= filters.priceRange.end.value);
  }
	if(sort){
    switch(sort.value){
			case "1":
