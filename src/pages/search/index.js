...
import {navigateToPDP, loadBadgeCart, navigateWithParams} from "../../utils/navigate";
import {getCategoriesAPI, getProductsByCategoryIdAPI, getFiltersAPI, getOtherProductsAPI, filterSortProductsAPI, getSortsAPI} from "../../services/index";
// @ts-ignore
Page({
	maxSearch: 5,
	data: {
	...
	async loadData(){
		...
		try {
			const [categories, products, otherProducts, filters, sorts, recentKeys] = await Promise.all([
				getCategoriesAPI(),
			...
			this.setData({
				categories, products, otherProducts, filters, sorts,
